...
def evaluate_policy(policy, eval_episodes=50):
  ...
  #desired = [0.1,-0.1,0.1]
  env.INIT_XYZS[0] = [0.1,0,0.4]
  #env.desired_pos = [0.1,0,0.4]
...
j = 0
desired = [[0,-0.15,0.4], [0.106,-0.106,0.4], [0.15,0,0.4], [0.103,0.109,0.4], [0,0.15,0.4], [-0.103,0.109,0.4], [-0.15,0,0.4], [-0.107,-0.105,0.4]]
#env.desired_pos = [0.1,-0.1,0.4]
#env.desired_pos = [0.1,0.1,0.4]
obs = env.reset()
#r = 0.15
...
for i in range(30*env.SIM_FREQ):
    ...
    obs, reward, done, info = env.step(action)
    #env.desired_pos = [-0.1,-0.1,0.4]
    if i % (30*env.SIM_FREQ/8) == 0:
       env.desired_pos = desired[j]
       j = j + 1
    #a += 360/(30*env.SIM_FREQ)
    #a += 5
    #x = r * cos(a)
    #y = r * sin(a)
    #print("x",x)
